...
?>
<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>РџРѕРЅРі вЂ” DonateCraft</title><link rel="stylesheet" href="style.css"><style>
...
<h1>рџЏ“ РџРѕРЅРі</h1>
<div class="game-info-bar"><div class="game-info-item"><span class="lbl">РЎС‡С‘С‚</span><span class="val" id="scoreDisplay">0</span></div><div class="game-info-item"><span class="lbl">РџСЂРѕРїСѓС‰РµРЅРѕ</span><span class="val" id="missDisplay">0</span></div><div class="game-info-item"><span class="lbl">Р РµРєРѕСЂРґ</span><span class="val" id="bestDisplay"><?= $bestScore ?></span></div></div>
...
function update() {
    ...
    if (ball.x - BALL_SIZE < 0) {
        ...
        if (aiScore >= MAX_MISSES) { gameMessage.textContent = 'рџµ РРіСЂР° РѕРєРѕРЅС‡РµРЅР°! Р’С‹ РїСЂРѕРёРіСЂР°Р»Рё ' + aiScore + '-' + playerScore; endGame(); return; }
        resetBall();
    }
    if (ball.x + BALL_SIZE > W) {
        playerScore++;
        scoreDisplay.textContent = playerScore;
        if (playerScore >= WIN_SCORE) { gameMessage.textContent = 'рџЋ‰ РџРѕР±РµРґР°! ' + playerScore + '-' + aiScore; endGame(); return; }
        resetBall();
    }
...
function endGame() {
    ...
    scoreDisplay.textContent = playerScore;
    if (!gameMessage.textContent) gameMessage.textContent = 'РРіСЂР° РѕРєРѕРЅС‡РµРЅР°!';

...
?>
<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Р”РёРЅРѕР·Р°РІСЂРёРє вЂ” DonateCraft</title><link rel="stylesheet" href="style.css"><style>
...
<div class="game-info-item"><span class="lbl">РЎРєРѕСЂРѕСЃС‚СЊ</span><span class="val" id="speedDisplay">1</span></div>
</div>
<div class="game-area"><canvas id="gameCanvas" width="600" height="250"></canvas></div>
<div class="game-controls"><button class="btn" onclick="startGame()">рџ”„ РЎС‚Р°СЂС‚ / РџСЂРѕР±РµР»</button></div>
</div></div>
...
function draw() {
    ...
    if (!running && !gameOver) {
        ...
        ctx.fillText('РќР°Р¶РјРё РџСЂРѕР±РµР» РґР»СЏ РїСЂС‹Р¶РєР°', canvas.width / 2, 80);
    }
    ...
    if (gameOver) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText('рџ’Ђ РРіСЂР° РѕРєРѕРЅС‡РµРЅР°!', canvas.width / 2, canvas.height / 2 - 20);
        ctx.fillStyle = '#ffaa33';
        ctx.font = '18px Inter, sans-serif';
        ctx.fillText('РЎС‡С‘С‚: ' + score, canvas.width / 2, canvas.height / 2 + 15);
    }
}
